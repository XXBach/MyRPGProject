...
public class PathFinding
{
    ...
    public List<PathNode> FindPath(int startXCoordinate, int startYCoordinate, int endXCoordinate, int endYCoordinate, bool canMoveDiagonal = false)
    {
        PathNode startNode = _grid.GetGridObject(startXCoordinate, startYCoordinate);
        PathNode endNode = _grid.GetGridObject(endXCoordinate, endYCoordinate);
        ...
        while (_openList.Count > 0) {
            ...
            this._closedList.Add(currentNode);
            this._openList.Remove(currentNode);

            foreach (PathNode neighborNode in GetNeighborList(currentNode, canMoveDiagonal))
            {
                if (this._closedList.Contains(neighborNode)) continue;
                if (neighborNode.State == NodeState.Blocked)
                {
                ...
                if (tentativeGCost < neighborNode.GCost)
                {
                    ...
                    if (!this._openList.Contains(neighborNode)) this._openList.Add(neighborNode);
                }
            }
        }
